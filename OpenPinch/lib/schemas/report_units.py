...
def _is_stateful_payload(value: Any) -> bool:
    return (
        isinstance(value, Mapping)
        and "values" in value
        and set(value).issubset(_STATEFUL_PAYLOAD_KEYS)
    )
...
def _value_array_to_list(value: Value) -> list[float]:
    return [float(item) for item in value.state_values.tolist()]
...
def _resolve_stateful_values(values: Any, *, idx: int | None = None) -> float | None:
    resolved = np.asarray(values, dtype=float).reshape(-1)
    if resolved.size == 0:
        raise ValueError("Stateful report values cannot be empty.")
    if resolved.size == 1:
        return _coerce_optional_float(resolved[0])

    resolved_idx = 0 if idx is None else int(idx)
    if resolved_idx < 0 or resolved_idx >= resolved.size:
        raise ValueError(
            f"idx {resolved_idx} is out of range for {resolved.size} state(s)."
        )
    return _coerce_optional_float(resolved[resolved_idx])


def _resolve_scalar_value(value: Any, *, idx: int | None = None) -> float | None:
    ...
    if isinstance(value, Value):
        ...
        return _resolve_stateful_values(value.state_values, idx=idx)
    if _is_numeric_scalar(value):
        return float(value)
    if _is_scalar_payload(value):
        return _coerce_optional_float(value.get("value"))
    if _is_stateful_payload(value):
        return _resolve_stateful_values(value.get("values"), idx=idx)
    ...
    if hasattr(value, "values") and hasattr(value, "unit"):
        return _resolve_stateful_values(getattr(value, "values"), idx=idx)
...
def split_report_value(
    ...
    *,
    ...
    """Return ``(value, unit)`` for scalar, stateful, or array report payloads."""
    value = _normalise_input_object(value)
    if value is None:
        return None, None
    ...
    if isinstance(value, Value):
        ...
        return _resolve_stateful_values(value.state_values, idx=idx), value.unit
    ...
    try:
        return _resolve_scalar_value(value, idx=idx), unit
