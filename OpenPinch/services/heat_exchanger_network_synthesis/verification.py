"""Validation shell for accepted HEN synthesis design payloads."""
...
def verify_synthesis_result(
    ...
    """Return contract failures for an accepted in-memory HEN design result."""
    failures: list[str] = []
    if not result.network.exchangers:
        failures.append("accepted HEN design must contain at least one exchanger")
    ...
    if (
        ...
        failures.append("accepted HEN design must include total annual cost metadata")
