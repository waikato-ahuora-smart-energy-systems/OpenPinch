...
def _prepare_heat_pump_target_inputs(
    ...
    *,
    ...
    T_hot, T_cold, dtcont_hp = _apply_temperature_shift_for_heat_pump_stream_dtmin_cont(
        T_vals, zone_config.DTMIN_HP, is_direct_integration
    )
    ...
    return HeatPumpTargetInputs(
        ...
        if zone_config.PRICE_RATIO_ELE_TO_FUEL > 0.0
        else 1,
...
def _apply_temperature_shift_for_heat_pump_stream_dtmin_cont(
    ...
    """
    dT_shift = (
        dtmin_hp * 0.5 if is_direct_integration else dtmin_hp * 1.5
    )  # utility streams are real temperatures???
...
def _get_H_col_till_target_Q(
    ...
    *,
    ...
    mask = np.where(np.abs(H_vals) >= Q_max, 1.0, 0.0)
...
def _balance_hot_and_cold_heat_loads_with_ambient_air(
    ...
    if is_heat_pumping:
        if delta_H > tol:
            ...
            mask = np.where(
                T_hot <= (T_env - dT_env_cont - dt_phase_change - dtcont), delta_H, 0.0
            )
            H_hot -= mask
            Q_amb_max = delta_H
        elif delta_H < -tol:
...
def _compute_multi_temperature_carnot_hp_opt_obj(
    ...
    *,
    ...
    scale_cond = (
        min(work_hp_from_evap / work_hp_from_cond, 1)
        if work_hp_from_cond > 0.0
        else 0.0
    )
    Q_cond *= scale_cond
    scale_evap = (
        min(work_hp_from_cond / work_hp_from_evap, 1)
        if work_hp_from_evap > 0.0
        else 0.0
    )
    Q_evap *= scale_evap
...
def _compute_multi_simple_carnot_hp_opt_obj(
    ...
    *,
    ...
    Q_ext = max(args.Q_hp_target - Q_cond.sum(), 0.0)  # e.g., direct electric heating
    obj = _calc_obj(
        work_hp.sum() - work_he.sum(), Q_ext, args.Q_hp_target, args.price_ratio, p
    )
...
def _compute_brayton_hp_system_performance(
    ...
    if 0:
        ...
        plot_multi_hp_profiles_from_results(
            ...
            hp_hot_streams,
            hp_cold_streams,
            title=f"T_hi {T_comp_out} -> {float(obj), float(c / args.Q_hp_target)}",
        )
