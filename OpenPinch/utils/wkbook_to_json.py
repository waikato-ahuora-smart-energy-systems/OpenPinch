...
def _validate_stream_data(sd: pd.DataFrame):
    """Backward-compatible alias. Use ``validate_stream_data`` instead."""
    return validate_stream_data(sd)


def _validate_utilities_data(ud: pd.DataFrame):
    """Backward-compatible alias. Use ``validate_utility_data`` instead."""
    return validate_utility_data(ud)
