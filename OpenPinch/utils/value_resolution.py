"""Helpers for resolving scalar and stateful value payloads."""
...
_STATEFUL_PAYLOAD_KEYS = {"values", "state_ids", "weights", "unit"}
...
def _is_stateful_value_object(value: Any) -> bool:
...
def _is_stateful_value_payload(value: Any) -> bool:
    if not isinstance(value, Mapping):
        return False
    keys = set(value)
    return keys.issubset(_STATEFUL_PAYLOAD_KEYS) and (
...
def _is_zone_value_map(value: Any) -> bool:
    return (
        ...
        and not _is_stateful_value_payload(value)
    ...
    *,
    ...
    if idx is None:
        if not default_allowed:
            raise ValueError("idx is required for stateful values.")
        idx = 0
    ...
    if idx >= resolved.size:
        raise ValueError(f"idx {idx} is out of range for {resolved.size} state(s).")
    return _coerce_optional_float(resolved[idx])
...
def get_state_value(
    ...
    *,
    ...
    if isinstance(value, Mapping):
        ...
        if _is_stateful_value_payload(value):
            if "values" not in value:
                raise KeyError("values")
            return _resolve_stateful_values(
                value.get("values"),
                idx=idx,
        ...
        raise TypeError("Unsupported mapping for state value resolution.")
    ...
    if _is_stateful_value_object(value):
        return _resolve_stateful_values(
    ...
    raise TypeError(
        ...
        "or serialized scalar/stateful value payload."
    )
...
def get_scalar_value(
    ...
    *,
    ...
    return get_state_value(value, idx=idx, default_allowed=default_allowed)
...
def _evaluate_nested_spec(
    ...
    *,
    ...
    if isinstance(spec, Mapping) and not (
        _is_scalar_value_payload(spec) or _is_stateful_value_payload(spec)
    ):
        return evaluate_value_spec(spec, zone_name=zone_name, idx=idx)
    return get_scalar_value(spec, idx=idx)
...
def _resolve_math_base(
    ...
    *,
    ...
    try:
        base = _evaluate_nested_spec(spec, zone_name=zone_name, idx=idx)
...
def evaluate_value_spec(
    ...
    *,
    ...
    """Evaluate the legacy value mini-DSL using explicit idx-based resolution."""
    spec = _normalise_input_object(spec)
    if spec is None:
        return (
            None if default_value is None else get_scalar_value(default_value, idx=idx)
        )
    ...
    if _is_scalar_value_payload(spec) or _is_stateful_value_payload(spec):
        return get_scalar_value(spec, idx=idx)
    ...
    if operator_keys:
        ...
        value = _evaluate_nested_spec(base_spec, zone_name=zone_name, idx=idx)
        if value is None:
            return None
        ...
        if operator == "log":
            base = _resolve_math_base(spec[operator], zone_name=zone_name, idx=idx)
        ...
        if operator == "min":
            return min(
                value,
                _evaluate_nested_spec(spec[operator], zone_name=zone_name, idx=idx),
            )
        if operator == "max":
            return max(
                value,
    ...
    if "value" in spec:
        return _evaluate_nested_spec(spec["value"], zone_name=zone_name, idx=idx)

    if default_value is not None:
        return get_scalar_value(default_value, idx=idx)

    raise KeyError("value")


def resolve_value_array(value: Any) -> np.ndarray:
    """Resolve scalar or stateful value-like payloads into a 1-D float array."""
    value = _normalise_input_object(value)
    if value is None:
    ...
    if isinstance(value, Value):
        return np.asarray(value.state_values, dtype=float).reshape(-1)
    ...
    if hasattr(value, "units"):
        return np.asarray(Value(value).state_values, dtype=float).reshape(-1)
