"""Contracts for inverse heat-recovery approach-temperature targeting."""
...
def _result(**overrides) -> HeatRecoveryApproachResult:
    payload = {
        "scope": "Site/Process",
        "period_id": "0",
        "approach_temperature": {"value": 10.0, "unit": "delta_degC"},
    ...
    return HeatRecoveryApproachResult.model_validate(payload)
...
def test_result_is_frozen_strict_finite_and_json_round_trippable() -> None:
    ...
    assert result.approach_temperature == HeatRecoveryQuantity(
        value=10.0,
        unit="delta_degC",
    )
    ...
    with pytest.raises(ValidationError, match="finite"):
        _result(approach_temperature={"value": float("inf"), "unit": "delta_degC"})
