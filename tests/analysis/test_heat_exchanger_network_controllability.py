...
from OpenPinch.analysis.heat_exchanger_networks import (
    controllability as controllability_service,
)
from OpenPinch.analysis.heat_exchanger_networks.controllability import (
    quantify_heat_exchanger_network_controllability,
)
from OpenPinch.contracts.controllability import (
...
def test_controllability_service_reports_low_thermal_margin() -> None:
    ...
    result = quantify_heat_exchanger_network_controllability(
        network,
        minimum_approach_temperature=5.0,
    )
...
def test_partial_controllability_case_distinguishes_projected_outputs() -> None:
    ...
    single_projection = quantify_heat_exchanger_network_controllability(
        monotubular_like
    )
    coupled_projection = quantify_heat_exchanger_network_controllability(
        two_stream_like
    )
...
def test_output_feedback_case_does_not_require_temperature_measurements() -> None:
    ...
    result = quantify_heat_exchanger_network_controllability(
        HeatExchangerNetwork(exchangers=(exchanger,))
    )
...
def test_ill_conditioned_partial_gramian_proxy_is_reported() -> None:
    ...
    result = quantify_heat_exchanger_network_controllability(
        network, condition_warning_threshold=10.0
    )
