...
import OpenPinch.analysis.heat_pumps.direct_mvr.units as direct_mvr_units
from OpenPinch.analysis.heat_pumps.direct_mvr.execution import (
...
DIRECT_MVR_STAGE_CASES = FIXTURES_ROOT / "process_mvr_edge_cases.json"
...
def test_direct_mvr_value_stage_and_stage_count_helpers_cover_scalar_edges():
    ...
    with pytest.raises(ValueError, match="positive integer"):
        direct_mvr_execution.coerce_positive_mvr_stage_count("not-a-number")

    assert direct_mvr_units.stage_mass_flow(explicit_mass_stage) == pytest.approx(2.5)
    assert direct_mvr_units.stage_mass_flow(zero_delta_stage) == 0.0
    assert direct_mvr_units.stage_mass_flow(positive_stage) == pytest.approx(0.5)
    assert direct_mvr_units.value_at_index(None, 0) is None
    assert direct_mvr_units.value_at_index(12.5, 99) == pytest.approx(12.5)
    assert direct_mvr_units.value_at_index(Value([1.0, 2.0], "kW"), 1, unit="W") == (
        pytest.approx(2000.0)
    )
    assert direct_mvr_units.stage_pressure_to_pascal(1.2, "bar") == pytest.approx(
        120000.0
    )
...
def test_direct_mvr_state_property_fallback_and_failure_paths(monkeypatch):
    ...
    monkeypatch.setattr(direct_mvr_thermodynamics, "PropsSI", saturated_property)

    assert direct_mvr_thermodynamics.state_property_at_temperature_pressure(
        fluid="Water",
        output="H",
        t_c=100.0,
        p_kpa=101.325,
    ...
    monkeypatch.setattr(
        ...
        "PropsSI",
        distant_saturation_temperature,
    )
    with pytest.raises(ValueError, match="ambiguous saturation state"):
        direct_mvr_thermodynamics.state_property_at_temperature_pressure(
            fluid="Water",
            output="H",
            t_c=100.0,
            p_kpa=101.325,
...
def test_direct_mvr_pressure_search_and_profile_helpers_cover_failure_edges(
    ...
    with pytest.raises(ValueError, match="no heat-release enthalpy range"):
        direct_mvr_thermodynamics.build_cooling_temperature_enthalpy_curve(
            fluid="Air",
            outlet_pressure=101325.0,
            hot_supply_enthalpy=100.0,
            target_enthalpy=100.0,
        )

    monkeypatch.setattr(
        direct_mvr_thermodynamics,
        "actual_outlet_at_pressure",
        lambda *_args, **_kwargs: (100.0, 300.0),
    )
    with pytest.raises(ValueError, match="feasible MVR discharge pressure"):
        direct_mvr_thermodynamics.find_pressure_for_actual_discharge(
            fluid="Air",
            inlet_pressure=101325.0,
            inlet_enthalpy=100.0,
            inlet_entropy=1.0,
            target_discharge_kelvin=500.0,
...
def test_direct_mvr_liquid_injection_property_failure_falls_back_to_dry_stage(
    ...
    monkeypatch.setattr(direct_mvr_execution, "PropsSI", fake_props)
    monkeypatch.setattr(direct_mvr_thermodynamics, "PropsSI", fake_props)
    monkeypatch.setattr(
