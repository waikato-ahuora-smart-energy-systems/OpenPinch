...
def test_public_synthesis_exports_are_openpinch_native():
    ...
    concrete_types = {
        ...
        HeatExchangerNetworkSynthesisResult: ("OpenPinch.contracts.synthesis.result"),
        HeatExchangerNetworkSynthesisTask: "OpenPinch.contracts.synthesis.task",
    }
    assert all(
        model.__module__ == module_name for model, module_name in concrete_types.items()
    )
    ...
    for compatibility_module in ("methods", "results", "tasks"):
        assert (
            importlib.util.find_spec(
                f"OpenPinch.contracts.synthesis.{compatibility_module}"
            )
            is None
        )
    assert isinstance(
...
def test_openhens_compatibility_surfaces_are_absent():
    ...
    modules = [
        OpenPinch,
        __import__("OpenPinch.contracts.synthesis", fromlist=["*"]),
        __import__("OpenPinch.analysis.heat_exchanger_networks", fromlist=["*"]),
    ]
