...
from OpenPinch.analysis.heat_exchanger_networks.results.selection import (
    ranked_networks,
    select_network,
)
from OpenPinch.contracts.synthesis.result import HeatExchangerNetworkSynthesisResult
...
from OpenPinch.presentation.network_grid._constants import (
    _COLD_STREAM_COLOR,
    _COLD_UTILITY_COLOR,
    _HOT_STREAM_COLOR,
    _HOT_UTILITY_COLOR,
...
from OpenPinch.presentation.network_grid.dependencies import (
    MissingNetworkGridDiagramDependencyError,
    require_network_grid_diagram_dependency,
)
from OpenPinch.presentation.network_grid.geometry import build_grid_model
from OpenPinch.presentation.network_grid.labels import add_label
from OpenPinch.presentation.network_grid.plotly import (
    _PlotlyAxes,
    _PlotlyLine,
    _SplitGroup,
)
from OpenPinch.presentation.network_grid.rendering import _PlotlyGridRenderer
from OpenPinch.presentation.network_grid.results import (
    HeatExchangerNetworkGridDiagram,
    build_result_grid_diagram,
)
from OpenPinch.presentation.network_grid.service import build_grid_diagram
from OpenPinch.presentation.network_grid.state import HeatExchangerNetworkGridModel
from OpenPinch.presentation.network_grid.temperatures import (
    midpoint_temperature,
    recovery_match_x_pair,
    split_adjusted_temperature_x,
    temperature_scale,
    x_from_temperature,
)
...
def test_grid_diagram_ranks_successful_networks_by_accepted_method() -> None:
    ...
    diagram = build_result_grid_diagram(result, solution_rank=1)
...
def test_result_selects_ranked_network() -> None:
    ...
    select_network(result, solution_rank=2)
...
def test_grid_diagram_right_side_split_temperature_labels_move_right() -> None:
    ...
    diagram = build_result_grid_diagram(
        _result_for_network(_right_side_split_network()),
        solution_rank=1,
    )
...
def test_grid_diagram_hot_utility_intermediate_temperature_uses_utility_inlet() -> None:
    ...
    diagram = build_result_grid_diagram(
        _result_for_network(_supply_side_hot_utility_network()),
        solution_rank=1,
    )
...
def test_grid_diagram_orders_stage_matches_by_mid_temperature_attributes() -> None:
    ...
    diagram = build_result_grid_diagram(
        _result_for_network(_out_of_order_midpoint_network()),
        solution_rank=1,
    )
    recovery_lines = {
...
def test_grid_diagram_height_is_proportional_to_lane_count() -> None:
    ...
    split_four_stream = build_result_grid_diagram(_result(), solution_rank=1)
    plain_four_stream = build_result_grid_diagram(
        _result_for_network(_plain_four_stream_network()),
        solution_rank=1,
    )
    two_stream = build_result_grid_diagram(
        _result_for_network(_two_stream_network()),
        solution_rank=1,
    )
...
def test_grid_diagram_marker_radius_is_bounded_by_row_pitch() -> None:
    ...
    diagrams = (
        build_result_grid_diagram(_result(), solution_rank=1),
        build_result_grid_diagram(
            _result_for_network(_plain_four_stream_network()),
            solution_rank=1,
        ),
...
def test_grid_diagram_label_offsets_use_geometry_clearance() -> None:
    ...
    diagram = build_result_grid_diagram(_result(), solution_rank=1)
...
def test_grid_renderer_helper_edges_cover_temperature_and_split_fallbacks() -> None:
    ...
    assert recovery_match_x_pair(renderer, recovery_match) == (0.0, 0.0)
    ...
    assert x_from_temperature(fallback_renderer, None) == pytest.approx(0.0)
    assert x_from_temperature(fallback_renderer, 100.0) == pytest.approx(0.5)
    ...
    assert temperature_scale(empty_model) == (0.0, 1.0)
    assert midpoint_temperature(None, None) is None
    ...
    assert split_adjusted_temperature_x(0.5, 0.6, split_group) == pytest.approx(0.5)
...
def test_grid_renderer_duplicate_labels_without_offsets_stack_downward() -> None:
    ...
    add_label(renderer, 0.25, 0.5, "A")
    add_label(renderer, 0.25, 0.5, "B")
...
def test_grid_diagram_can_temperature_scale_stream_x_positions() -> None:
    ...
    staged = build_result_grid_diagram(result, solution_rank=1)
    scaled = build_result_grid_diagram(result, solution_rank=1, temperature_scaled=True)
...
def test_grid_diagram_temperature_scaled_uses_process_midpoints_for_recovery() -> None:
    ...
    scaled = build_result_grid_diagram(
        _result_for_network(_temperature_scaled_network()),
        solution_rank=1,
