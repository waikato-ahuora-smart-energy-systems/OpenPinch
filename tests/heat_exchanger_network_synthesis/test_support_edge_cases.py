...
from OpenPinch.services.heat_exchanger_network_synthesis import (
    heat_exchanger_network_synthesis_entry as entry,
)
...
from OpenPinch.services.heat_exchanger_network_synthesis.common.reporting import (
    exports,
    ranking,
)
from OpenPinch.services.heat_exchanger_network_synthesis.common.results import (
    assembly,
    seeds,
)
...
def test_service_context_solver_dependency_and_entry_guards(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "OpenPinch.services.heat_exchanger_network_synthesis.common.service_context.rank_unique_network_outcomes",
        lambda _result: (),
    )
    monkeypatch.setattr(
        "OpenPinch.services.heat_exchanger_network_synthesis.common.service_context.verify_synthesis_result",
...
def test_stagewise_recovery_binary_and_scalar_value_fallbacks():
    ...
    assert stagewise._value(7.5) == pytest.approx(7.5)
    assert stagewise._value(
    ...
    assert stagewise._value(FloatOnly()) == pytest.approx(10.5)
...
def test_stagewise_temperature_checks_report_infeasible_active_matches():
    ...
    assert stagewise._check_temperatures(
        inactive_case,
        rel_tol=1e-6,
        abs_tol=1e-6,
        q_tol=1e-2,
    ...
    assert (
        stagewise._check_state_temperatures(
            state_nonisothermal,
            rel_tol=1e-6,
            abs_tol=1e-6,
            q_tol=1e-2,
...
def test_stagewise_shared_area_cost_checks_each_failure_path():
    ...
    assert stagewise._check_area_costs(no_recovery) is True
    ...
    assert stagewise._check_area_costs(bad_recovery) is False
    ...
    assert stagewise._check_area_costs(bad_hot_utility) is False
    ...
    assert stagewise._check_area_costs(bad_cold_utility) is False
