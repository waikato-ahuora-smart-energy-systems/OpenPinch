...
from OpenPinch.services.heat_exchanger_network_synthesis.unit_models import stagewise
from OpenPinch.services.heat_exchanger_network_synthesis.unit_models.stagewise import (
    StageWiseModel,
)

FIXTURE_PATH = (
    Path(__file__).resolve().parents[1] / "fixtures" / "stagewise_helper_cases.json"
)
...
def test_stagewise_temperature_verification_from_static_cases(case_name, expected):
    assert stagewise._check_temperatures(_case(case_name)) is expected


def test_stagewise_utility_cost_verification_handles_source_and_period_cases():
    assert stagewise._check_utility_costs(_case("utility_cost_source")) is True
    assert stagewise._check_utility_costs(_case("utility_cost_state")) is True

    bad_source = _case("utility_cost_source")
    bad_source.hu_cost_total = 999.0
    assert stagewise._check_utility_costs(bad_source) is False

    bad_state = _case("utility_cost_state")
    bad_state.cu_cost_total = 999.0
    assert stagewise._check_utility_costs(bad_state) is False


def test_stagewise_area_cost_verification_handles_source_and_shared_cases():
    assert stagewise._check_area_costs(_source_area_case()) is True
    assert stagewise._check_area_costs(_source_area_case(model_area=0.0)) is False

    assert stagewise._check_area_costs(_shared_area_case()) is True

    failed_recovery = _shared_area_case()
    failed_recovery.area_r_shared = [[[0.0]]]
    assert stagewise._check_area_costs(failed_recovery) is False

    failed_hot_utility = _shared_area_case()
    failed_hot_utility.area_hu_shared = [0.0]
    assert stagewise._check_area_costs(failed_hot_utility) is False

    failed_cold_utility = _shared_area_case()
    failed_cold_utility.area_cu_shared = [0.0]
    assert stagewise._check_area_costs(failed_cold_utility) is False

    failed_total = _shared_area_case()
    failed_total.recovery_area_cost_total = 999.0
    assert stagewise._check_area_costs(failed_total) is False
...
def test_stagewise_candidate_helpers_cover_invalid_and_verified_paths():
    assert stagewise._count_allowed_matches([[[1, [1], 0, [0]]]]) == 2
    assert stagewise._is_usable_evolution_candidate(None) is False
    assert (
        stagewise._is_usable_evolution_candidate(_VerifiedCandidate(m_success=0))
        is False
    )
    assert (
        stagewise._is_usable_evolution_candidate(
...
def test_stagewise_value_coercion_accepts_solver_like_shapes():
    assert stagewise._value(1) == 1.0
    assert stagewise._value([2.5]) == 2.5
    assert stagewise._value(SimpleNamespace(VALUE=[3.5])) == 3.5
    assert stagewise._value(SimpleNamespace(value=[4.5])) == 4.5
    assert stagewise._value(SimpleNamespace(VALUE=SimpleNamespace(value=5.5))) == 5.5
    assert (
        stagewise._value(SimpleNamespace(VALUE=SimpleNamespace(value=_FloatOnly(6.5))))
        == 6.5
    )
    assert math.isclose(stagewise._value(_FloatOnly(7.5)), 7.5)
...
def test_stagewise_post_process_and_verify_failure_edges(monkeypatch):
    ...
    monkeypatch.setattr(stagewise, "_check_temperatures", lambda case: False)
    monkeypatch.setattr(stagewise, "_check_utility_costs", lambda case: False)
    monkeypatch.setattr(stagewise, "_check_area_costs", lambda case: False)
