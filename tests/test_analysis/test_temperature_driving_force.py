...
def _base_curves():
    T_hot = np.array([400.0, 330.0, 300.0])
    H_hot = np.array([0.0, 120.0, 300.0])
    T_cold = np.array([200.0, 240.0, 280.0])
    H_cold = np.array([0.0, 80.0, 300.0])
...
def test_get_temperature_driving_forces_balanced_returns_expected_values():
    ...
    expected_h = pd.Series([0.0, 0.0, 80.0, 120.0], dtype=float)
    expected_delta_T1 = np.array([200.0, 200.0, 113.33333333333333])
    expected_delta_T2 = np.array([200.0, 113.33333333333333, 82.72727272727273])
    expected_dh = np.array([0.0, -80.0, -40.0])

    pd.testing.assert_series_equal(result["h_vals"], expected_h, check_names=False)
