...
def test_raises_on_utility_zone_input():
    ...
    with pytest.raises(ValueError, match="Pinch analysis does not apply to Utility Zones."):
...
from OpenPinch.analysis.data_preparation import _validate_config_data_completed, _validate_streams_passed_in
...
def test_unexpected_zone_type_raises():
    ...
    with pytest.raises(ValueError, match="Zone name and type could not be identified correctly."):
