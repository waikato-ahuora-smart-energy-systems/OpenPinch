...
from OpenPinch.lib.enums import PT, TT, HPRcycle
...
def test_solve_hpr_multiperiod_placement_fails_when_any_period_fails(monkeypatch):
    monkeypatch.setattr(
        hp_shared,
        "multiminima",
        lambda **kwargs: (np.array([[1.0]]), np.array([0.0])),
    )
    cases = [
        HPRPeriodCase(
            period_id="p0",
    ...
    with pytest.raises(ValueError, match="failed to return an optimal result"):
        hp_shared.solve_hpr_multiperiod_placement(
            f_obj=objective,
            x0_ls=None,
            bnds=[(0.0, 1.0)],
            args=mp_args,
