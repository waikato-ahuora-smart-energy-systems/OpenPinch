...
def test_set_initial_values_for_condenser_and_evaporator_even_spacing():
    ...
    T_cond, T_evap = _set_initial_values_for_condenser_and_evaporator(
        2, 3, T_bnds, T, H_hot, T, H_cold
    )
    expected_cond = [200., 140.]
    expected_evap = [60., 20., 20.]
...
def test_convert_idx_to_temperatures_maps_indices_to_bounds():
    ...
    np.testing.assert_allclose(result["HU"], (T_cold[1], T_cold[3]))
    np.testing.assert_allclose(result["CU"], (T_hot[0], T_hot[2]))
