...
def test_get_multi_temperature_carnot_stage_duties_and_work_returns_entropic_mean_cop():
    ...
    cop = cycle_results["cop"]
    Q_cond_out = cycle_results["Qc"]
    Q_evap_out = cycle_results["Qe"]
    work_hpr = cycle_results["w_hpr"]
...
def test_get_multi_simple_carnot_stage_duties_and_work_shares_hot_profile_across_modes():
    ...
    np.testing.assert_allclose(cycle_results["Qe"], np.array([expected_Q_hx, available_he]))
...
def test_map_x_to_T_returns_expected_descending_temperatures():
    ...
    result_T = _map_x_arr_to_T_arr(x, T_0, T_1)

    np.testing.assert_allclose(result_T, np.array([190.0, 172.0, 150.4]))

    result_x = _map_T_arr_to_x_arr(result_T, T_0, T_1)
...
def test_get_reduced_bckgrd_cascade_till_Q_target_interpolates_to_target():
    ...
    np.testing.assert_allclose(H_out[0], Q_hpr_target)
...
def test_parse_multi_temperature_carnot_cycle_state_variables_returns_expected_profiles():
    ...
    x = np.array([0.5, 0.5, 0.5, 0.25, 0.5])
...
def test_parse_multi_temperature_carnot_cycle_state_variables_respects_cond_evap_split_sizes():
    ...
    x = np.array([-0.1, 0.2, 0.5, 0.8, 0.4])
