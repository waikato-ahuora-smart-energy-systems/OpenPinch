...
from OpenPinch.application._problem.output import result_extraction as re
...
from OpenPinch.domain.targets import UtilitySummaryTarget
from OpenPinch.domain.zone import Zone


def _summary_target(name: str) -> UtilitySummaryTarget:
    return UtilitySummaryTarget(
        name=name,
        type="Direct Integration",
        hot_utility_target=10.0,
        cold_utility_target=9.0,
...
def test_report_and_utility_extractors_and_extract_results(monkeypatch):
    ...
    root._targets = {"t0": _summary_target("root_target")}
    child._targets = {"t1": _summary_target("child_target")}

    report = re._get_report(root)
    assert [item["name"] for item in report] == ["root_target", "child_target"]
