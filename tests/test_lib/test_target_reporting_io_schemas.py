...
from OpenPinch.lib.enums import ST, TT, FluidPhase
from OpenPinch.lib.enums import ProblemTableLabel as PT
from OpenPinch.lib.schemas import io, reporting, targets

FIXTURE_PATH = (
    Path(__file__).resolve().parents[1]
    / "fixtures"
    / "target_reporting_schema_cases.json"
)
...
def test_target_name_validation_and_base_graph_helpers():
    ...
    dumpable = SimpleNamespace(
        model_dump=lambda mode: {"nested": Value(1.0, "kW")},
    )
    assert targets._serialise_report_data(dumpable) == {
        "nested": {"value": 1.0, "unit": "kW"}
    }
...
def test_utility_summary_and_direct_target_reporting_round_trip():
    ...
    result = target.to_target_results(isTotal=True)
    ...
    serialised = target.serialize_json(isTotal=True)
    ...
    assert total_process.to_target_results().Qh.value == pytest.approx(1.0)
...
def test_total_site_and_heat_pump_target_reporting_include_special_fields():
    ...
    total_site_result = total_site.to_target_results()
    ...
    hpr_result = hpr.to_target_results()
