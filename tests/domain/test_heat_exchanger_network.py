...
import OpenPinch.analysis.heat_exchanger_networks.controllability as controllability
import OpenPinch.presentation.network_grid.service as grid_diagram
...
def test_network_services_accept_domain_network(monkeypatch):
    ...
    assert grid_diagram.build_grid_diagram(
        network,
        period_id=network.resolve_period_id(),
    ) == {"grid": "ok"}
    assert controllability.quantify_heat_exchanger_network_controllability(
        network,
        period_id=network.resolve_period_id(),
        mode="steady",
    ) == {"rank": 1}
...
def test_multiperiod_queries_require_explicit_period_identity():
    ...
    with pytest.raises(ValueError, match="period_id is required"):
        network.total_duty()
    with pytest.raises(ValueError, match="period_id is required"):
        grid_diagram.build_grid_diagram(network)
    with pytest.raises(ValueError, match="period_id is required"):
        controllability.quantify_heat_exchanger_network_controllability(network)
