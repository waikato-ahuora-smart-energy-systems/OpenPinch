...
from OpenPinch.application._problem.input.validation import validate_problem_inputs
from OpenPinch.application.workspace import PinchWorkspace
from OpenPinch.contracts.input import TargetInput
from OpenPinch.domain._stream.linearisation import align_temperature_heat_profiles
...
def test_stream_segment_is_private_and_preserves_internal_model_name():
    ...
    assert importlib.util.find_spec("OpenPinch.classes") is None
    assert not hasattr(
        __import__("OpenPinch.domain.stream", fromlist=["*"]), "StreamSegment"
    )
    assert Stream.__module__ == "OpenPinch.domain.stream"
    assert StreamSegment.__module__ == "OpenPinch.domain._stream.segment"
